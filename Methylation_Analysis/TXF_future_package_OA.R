...
length(InfiniumMethylation$probeType)
...
cov
length(cov)#is the length of the TXF_Ruddy_2014
...
#this seems useless but I'm going to use a previous version of the TXF list to joing to the new binding ( something weird but works )
...
OA_cpglist_hypo#this is the name of the list
# we will need a list of CpGs hypermethylated
OA_cpglist_hyper
#here it would be grate to add the possibility to add more lists.....
...
#calculation
Frecuency_OA_cpglist_hypo_TXF<-as.data.frame(TFnames4cpg_v2(OA_cpglist_hypo))
Frecuency_OA_cpglist_hypo_TXF

Frecuency_OA_cpglist_hyper_TXF<-as.data.frame(TFnames4cpg_v2(OA_cpglist_hyper))
Frecuency_OA_cpglist_hyper_TXF
...
length(OA_cpglist_hypo)#
length(OA_cpglist_hyper)#
OA_cpglist_TXF_hypohyper<-merge(Frecuency_OA_cpglist_hypo_TXF, Frecuency_OA_cpglist_hyper_TXF, by.x="row.names",by.y="row.names",all.x=TRUE,all.y=TRUE)
OA_cpglist_TXF_hypohyper

#I use the work previously done with the Array distribution of the enrichment of the TXFs in the Array

Allfatesandtissues_TXF_hypohyper_ceros<-merge(as.data.frame(All_161_TXFs),OA_cpglist_TXF_hypohyper, by.x="Var1",by.y="Row.names",all.x=TRUE,all.y=TRUE)
Allfatesandtissues_TXF_hypohyper_ceros
#Now we calculate the percentages
Allfatesandtissues_TXF_hypohyper_ceros[,5]<-(as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[,3])*100)/length(OA_cpglist_hypo)
Allfatesandtissues_TXF_hypohyper_ceros[,6]<-(as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[,4])*100)/length(OA_cpglist_hyper)
Allfatesandtissues_TXF_hypohyper_ceros[,7]<-(as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[,2])*100)/length(InfiniumMethylation$probeType)#this is the total number of CpGs in the array
Allfatesandtissues_TXF_hypohyper_ceros
#now I add the columns for the not bindig cpgs that we will use for the statistical calculations
Allfatesandtissues_TXF_hypohyper_ceros[,8]<-length(OA_cpglist_hypo)-(as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[,3]))
Allfatesandtissues_TXF_hypohyper_ceros[,9]<-length(OA_cpglist_hyper)-(as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[,4]))
Allfatesandtissues_TXF_hypohyper_ceros[,10]<-length(InfiniumMethylation$probeType)-(as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[,2]))#this is the total number of CpGs in the array
Allfatesandtissues_TXF_hypohyper_ceros

#now we label the columns
colnames(Allfatesandtissues_TXF_hypohyper_ceros)<-c("TXF_Name", "Array_binding","OA_cpglist_hypo","OA_cpglist_hyper",
                                                    "%_OA_cpglist_hypo","%_OA_cpglist_hyper", "%_Array",
                                                    "not_bind_OA_cpglist_hypo","not_bind_OA_cpglist_hyper","not_bind_Array"
)
#here it would be nice that the name of the files inputed by the user were used as the labels for the columns ( currently I don't know how to do it)
write.table(Allfatesandtissues_TXF_hypohyper_ceros, file = "~/Documents/Newcastle Data/common files/Frequency CH_OB_HIPOA/Allfatesandtissues_TXF_hypohyper_ceros_CH_5_solo.txt", append = FALSE, quote = TRUE, sep = " ",
            eol = "\n", na = "NA", dec = ".", row.names = TRUE,
            col.names = TRUE, qmethod = c("escape", "double"))
# after this step using this matrix we can use the code for ploting. The plots that I did in excel are difficult to make them using R

#7) statisticts vs the array________________________________________________________________________________________________________________________________________________#
...
fisher_B_v2("STAT3","OA_cpglist_hypo" )# this function needs two arguments

############################################################################################################ For all the TXFs for a given list

fisher4all_lists_v2<-function(Loo){lapply(Allfatesandtissues_TXF_hypohyper_ceros[,1],fisher_B_v2,y=Loo)}
fisher4all_lists_v2("OA_cpglist_hypo")#all the p-values of the fisher test for a certain list of CPGs
...
#7) statisticts between hypo and hyper________________________________________________________________________________________________________________________________________________#
#this function is for a given TXF in a constant list of CPGs in this case is the column 3 
fisher_stats_v3<-function(x){
  numberofTXF<-match(x,Allfatesandtissues_TXF_hypohyper_ceros[,1])
  Convictions <-matrix(c(as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[numberofTXF,3]),
                         as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[numberofTXF,8]), 
                         as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[numberofTXF,4]), 
                         as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[numberofTXF,9])),
                       nrow = 2,
                       dimnames =
                         list(c("TXF-Bind", "TXF-Notbind"),
                              c("Observed", "array")))
...
fisher_stats_v3("RELA")

# this function is for determine a certain p-value for a given TXF and a given set of CpGs associated with a process
fisher_B_v3<-function(x,y){
  numberofTXF<-match(x,Allfatesandtissues_TXF_hypohyper_ceros[,1])
  numberoCpGlist<-match(y,colnames(Allfatesandtissues_TXF_hypohyper_ceros))
  Convictions <-matrix(c(  as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[numberofTXF, numberoCpGlist]),
                           as.numeric(Allfatesandtissues_TXF_hypohyper_ceros[numberofTXF, numberoCpGlist+5]), 
...
fisher_B_v3("STAT3","OA_cpglist_hypo" )# this function needs two arguments

############################################################################################################ For all the TXFs for a give list

fisher4all_lists_v3<-function(Loo){lapply(Allfatesandtissues_TXF_hypohyper_ceros[,1],fisher_B_v3,y=Loo)}
fisher4all_lists_v3("OA_cpglist_hypo")#all the p-values of the fisher test for a certain list of CPGs

#for all the TXFs and for all the list of Cpgs at the same time
Tabla_fisher_test<-sapply(colnames(Allfatesandtissues_TXF_hypohyper_ceros)[3:4],fisher4all_lists_v3)
rownames(Tabla_fisher_test)<-Allfatesandtissues_TXF_hypohyper_ceros[,1]
Tabla_fisher_test
write.table(Tabla_fisher_test, file = "~/Documents/Newcastle Data/common files/Fisher_of_all_the_sets.txt", append = FALSE, quote = TRUE, sep = " ",
            eol = "\n", na = "NA", dec = ".", row.names = TRUE,
...
cpgs4TXF<-function(X,Y){names(subsetByOverlaps(InfiniumMethylation[ X],TXF_Ruddy_2014[names(TXF_Ruddy_2014)==Y]))}
