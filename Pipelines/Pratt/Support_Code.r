...
png("PCA_ArrayID_Grouping.png",width=12.53,height=6.98,units="in",res=600) 
 
dev.off()

png("PostBC_PCA_ScanBatch_Grouping.png",width=12.53,height=6.98,units="in",res=600) 
plotPCA(na.omit(log2(batchCorrected_data)), groups=as.numeric(as.factor(Array_Batch)), 
        groupnames=levels(as.factor(Array_Batch)), addtext=sample_ID, pcs=c(1,2), 
        plot3d=F, main="Grouped By Array Batch - Post Batch Correction")
dev.off()
...
png("PostBC_ORm_PCA_Treatment_Grouping.png",width=12.53,height=6.98,units="in",res=600) 
plotPCA(normalised_data, groups=as.numeric(as.factor(Treatment)), 
        groupnames=levels(as.factor(Treatment)), pcs=c(1,2), 
        plot3d=F, main="Grouped By Treatment - Post Batch Correction", addtext=sample_ID)
dev.off()
...
png("PCA_RNAAmp_Grouping_Normalised.png",width=12.53,height=6.98,units="in",res=600) 
plotPCA(na.omit(log2((batchCorrected_data))), groups=as.numeric(as.factor(RNA_Batch)), 
        groupnames=levels(as.factor(RNA_Batch)), pcs=c(1,2), 
        plot3d=F, main="Grouped By RNA Amplification Batches", squarepca=F, addtext=sample_ID)#xlim=c(-16,16), ylim=c(-14,12))
dev.off()

png("PostBC_PCA_RNAAmp_Grouping_Normalised.png",width=12.53,height=6.98,units="in",res=600) 
plotPCA((batchCorrected_data), groups=as.numeric(as.factor(RNA_Batch)), 
        groupnames=levels(as.factor(RNA_Batch)), pcs=c(1,2), 
        plot3d=F, main="Grouped By RNA Amplification Batches - Post Batch Correction", squarepca=F, addtext=sample_ID)#xlim=c(-16,16), ylim=c(-14,12))
dev.off()
