...
file_Names          <- list.files()[1:24]
raw_data            <- lumiR.batch(file_Names)
...
gene_list   = topTable(fit2, coef = "NOF - OA", number = nrow(anno_df), adjust.method="BH")
...
gene_list_complete = topTable(fit2, coef = "NOF - OA", p.value=0.01, lfc=log2(2), number = nrow(anno_df), adjust.method="BH")
...
top_5_go = sig_go_id#[1:50]
top_5_p = go_fdr[go_fdr < 0.01]#[1:50]
amigo_tree = getAmigoTree(top_5_go, pvalues=top_5_p, filename='RamiGo_All.png')
###END RAMI_GO
